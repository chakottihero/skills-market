          ...
          <div className="space-y-3">
            {downloads.map((d) => (
              <div key={d.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-xl px-4 py-3">
                <div>
                  <Link href={`/skills/${d.skill_id}`} className="font-medium text-gray-900 hover:text-purple-600 text-sm">
                    {d.skills?.title ?? d.skill_id}
                  </Link>
                  <p className="text-xs text-gray-400">{d.created_at.slice(0, 10)}</p>
                </div>
          ...
          <div className="space-y-3">
            {purchases.map((p) => (
              <div key={p.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-xl px-4 py-3">
                <div>
                  <Link href={`/skills/${p.skill_id}`} className="font-medium text-gray-900 hover:text-purple-600 text-sm">
